...
class MLIRGraphDefImportTest(test.TestCase):
  ...
  def testGraphDefToLinalg(self):
    ...
    mlir_tf = import_graphdef(
        tf_graph_def,
        TF_TO_LINALG_PIPELINE,
        False,
        input_names=["lhs", "rhs"],
        input_data_types=["DT_FLOAT", "DT_FLOAT"],
        input_data_shapes=["", ""],
        output_names=["Add"])
    self.assertRegex(mlir_tf, r'linalg.generic')
...
class MLIRConcreteFunctionImportTest(test.TestCase):
  ...
  def testImportToLinalg(self):
    ...
    mlir_module = mlir.convert_function(concrete_function,
        pass_pipeline=TF_TO_LINALG_PIPELINE)
    self.assertRegex(mlir_module, r'linalg.generic')
